...
from mvector.models.tcres import TCResNet, TCResNetL
...
class MVectorTrainer(object):
    ...
    def __setup_model(self, input_size, is_train=False):
        ...
        elif self.configs.use_model == 'TCResNet':
            # 计算 n_channels
            self.channels = self.configs.model_conf.backbone['channels']
            self.channel_scale = self.configs.model_conf.backbone['channel_scale']
            self.configs.model_conf.backbone['n_channels'] = [int(cha * self.channel_scale) for cha in self.channels]
            self.backbone = TCResNet(input_size=input_size, **self.configs.model_conf.backbone)
        elif self.configs.use_model == 'TCResNetL':
            # 计算 n_channels
            self.channels = self.configs.model_conf.backbone['channels']
            self.channel_scale = self.configs.model_conf.backbone['channel_scale']
            self.configs.model_conf.backbone['n_channels'] = [int(cha * self.channel_scale) for cha in self.channels]
            self.backbone = TCResNetL(input_size=input_size, **self.configs.model_conf.backbone)
        else:
